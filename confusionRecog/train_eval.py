...
def collate(samples):
    ...
    trees, tokens, graphs, labels = map(list, zip(*samples))
    batched_graph = dgl.batch(graphs).to(ParameterConfig.device)
    batched_token = th.tensor(tokens).to(ParameterConfig.device)
    batched_label = th.tensor(labels).to(ParameterConfig.device)
    batched_tree = trees
    return batched_tree, batched_token, batched_graph, batched_label
...
def train(data_loader, loss_func, model, optimizer, scheduler, epoch):
    ...
    for iter, (tr, tk, bg, labels) in enumerate(data_loader):
        prediction = model(tr, tk, bg)
